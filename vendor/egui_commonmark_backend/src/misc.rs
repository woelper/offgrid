...
impl Style {
    pub fn to_richtext(&self, ui: &Ui, text: &str) -> RichText {
        ...
        // offgrid patch: egui has no font weights, so `strong()` only changes
        // the colour. If the app registered a font family named "bold" (its
        // UI font's Bold face), use it for strong text and headings h1–h4.
        // Inline code keeps its monospace face. Guarded, because epaint
        // panics on a family that is not bound to any font.
        let wants_bold = self.strong || matches!(self.heading, Some(0..=3));
        if wants_bold && !self.code {
            let bold = egui::FontFamily::Name("bold".into());
            if ui.fonts(|f| f.definitions().families.contains_key(&bold)) {
